...
export default function FinancialEducationScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-background p-2">
      {conteudos.map((item) => (
        <View key={item.id} className="bg-card rounded-lg mb-4">
          <Image
            source={ComoControlarGastosImg}
            className="w-full rounded-t-lg"
          />
          <View className="p-3 gap-2">
            <Text className="text-2xl font-bold text-white">{item.titulo}</Text>
            <Text className="text-white text-justify leading-5">
              {item.texto}
            </Text>
          </View>
        </View>
